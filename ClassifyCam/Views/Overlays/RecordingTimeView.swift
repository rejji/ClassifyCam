...
struct RecordingTimeView: PlatformView {
    ...
    var body: some View {
        VStack {
            /*
            if let soundIdentifier = appState.detectionStates.first?.0 {
                Text(soundIdentifier.displayName)
                    .padding(.trailing, 8)
            }
             */
            Text(time.formatted)
...
#Preview {
    RecordingTimeView(time: TimeInterval(floatLiteral: 500), appState: AppState())
