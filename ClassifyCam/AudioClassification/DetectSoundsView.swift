...
struct DetectSoundsView: View {
    ...
    var body: some View {
        VStack {
            ZStack {
                VStack {
                    HStack {
                        Spacer()
                        Button(action: {
                            classificationMode = .noClassify //this should better happen inside stopSoundClassification()
                ...
                VStack {
                    Text("Detecting Sounds").font(.headline).padding()
                ...
                VStack {
                    ...
                    Button(action: {
                        if isRecording {
                            ...
                            print("video recording is not yet on")
                        }
                    }) {
                        Text(isRecording ? "Start" : "Press Video Record Button")
                    }
                }
                .opacity(state.soundDetectionIsRunning ? 0.0 : 1.0)
                .disabled(state.soundDetectionIsRunning)
            ...
            /* No longer required after separate settings button
...
#Preview {
    @Previewable @State var mockConfig = AppConfiguration()
    @Previewable @State var mockClassificationMode = ClassificationMode.classify
    let mockState = AppState()

    return DetectSoundsView(
        state: mockState,
        isRecording: false,
        config: $mockConfig,
        classificationMode: $mockClassificationMode,
        configureAction: { print("Configure tapped") },
