...
struct SetupMonitoredSoundsView: View {
    ...
    var headerContent: some View {
        VStack {
            HStack {
                Button("Wishlist", action: {
                    isWishListPresented.toggle()
                })
                .padding(3)
                .sheet(isPresented: $isWishListPresented) {
                    WishListView()
                }
                ...
                Button(action: doneAction) {
                    Text("Done")
                        ...
                        .padding(.vertical, 5)
                        //.frame(maxWidth: .infinity) // Optional: Makes the button take full width
                }
...
struct SetupMonitoredSoundsView_Previews: PreviewProvider {
    @State static var selectedSounds = Set<SoundIdentifier>()

    static var previews: some View {
        SetupMonitoredSoundsView(
            querySoundOptions: {
                // Mock sound options for preview
                return [
                    SoundIdentifier(labelName: "Bird Chirping"),
                    SoundIdentifier(labelName: "Rain"),
                    SoundIdentifier(labelName: "Dog Barking"),
                    SoundIdentifier(labelName: "Thunder"),
                    SoundIdentifier(labelName: "Alarm")
                ]
            },
